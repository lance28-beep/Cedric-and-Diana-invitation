  ...
  useGesture(
    {
      onDragStart: ({ event }) => {
        ...
        if (pointerTypeRef.current === 'touch') {
          // No preventDefault on start to allow initial scroll decision
          // lockScroll() will be called if movement is horizontal/dome-related
        }
      ...
      onDrag: ({ event, last, velocity: velArr = [0, 0], direction: dirArr = [0, 0], movement }) => {
        ...
        // Smart scroll detection for touch
        if (pointerTypeRef.current === 'touch') {
          const dx = evt.clientX - startPosRef.current.x;
          const dy = evt.clientY - startPosRef.current.y;
          
          // If we haven't locked yet, decide based on direction
          if (!scrollLockedRef.current) {
            const isHorizontal = Math.abs(dx) > Math.abs(dy);
            const isVertical = Math.abs(dy) > Math.abs(dx);
            
            // Allow vertical scroll to pass through unless we're clearly interacting with the dome horizontally
            if (isVertical) {
              draggingRef.current = false; // Cancel dome drag
              return; // Let browser handle scroll
            }
            
            if (isHorizontal) {
              evt.preventDefault(); // Lock browser scroll
              lockScroll();
            }
          } else {
            evt.preventDefault();
      ...
      >
        ...
        >
          ...
          >
            ...
            <div
              ref={frameRef}
              className="viewer-frame h-full aspect-square flex items-center justify-center"
                        style={{
