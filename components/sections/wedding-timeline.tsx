...
export function WeddingTimeline() {
  return (
    ...
    >
      {/* Decorative corners - matching overall wedding motif */}
      {/* Top left corner */}
      ...
      <div className="relative z-10 text-center mb-6 sm:mb-9 md:mb-12 px-3 sm:px-4">
        {/* Small label */}
        <p
          className={`${cormorant.className} text-[0.7rem] sm:text-xs md:text-sm tracking-[0.3em] uppercase text-[#EFBE94] mb-2`}
        >
          Day Schedule
        </p>

        <h2 className={`${cinzel.className} text-3xl sm:text-4xl md:text-5xl lg:text-6xl font-normal text-[#EFBE94] mb-1.5 sm:mb-3 md:mb-4`}>
          Wedding Timeline
        </h2>

        <p className={`${cormorant.className} text-[11px] sm:text-sm md:text-base lg:text-lg text-[#EFBE94] max-w-xl mx-auto leading-relaxed px-2`}>
          A simple overview of the key moments of our day, from arrival to farewell.
        </p>

        {/* Simple divider */}
        <div className="flex items-center justify-center gap-2 mt-3 sm:mt-4">
          <div className="w-8 sm:w-12 md:w-16 h-px bg-gradient-to-r from-transparent via-[#EFBE94]/80 to-transparent" />
          <div className="w-1.5 h-1.5 bg-[#EFBE94]/90 rounded-full" />
          <div className="w-1.5 h-1.5 bg-[#EFBE94]/70 rounded-full" />
          <div className="w-1.5 h-1.5 bg-[#EFBE94]/90 rounded-full" />
          <div className="w-8 sm:w-12 md:w-16 h-px bg-gradient-to-l from-transparent via-[#EFBE94]/80 to-transparent" />
        </div>
      </div>
      ...
      <div className="relative z-10 max-w-6xl mx-auto px-3 sm:px-5 lg:px-8">
        ...
        <div className="hidden md:block absolute left-[4rem] md:left-[5rem] lg:left-[6rem] top-0 bottom-0 w-0.5 bg-gradient-to-b from-[#935F3B]/30 via-[#85441E]/55 to-[#B26A3B]/30 pointer-events-none">
          <div className="absolute top-0 left-1/2 -translate-x-1/2 w-1.5 h-1.5 rounded-full bg-[#85441E]" />
          <div className="absolute bottom-0 left-1/2 -translate-x-1/2 w-1.5 h-1.5 rounded-full bg-[#85441E]" />
        </div>

        {/* Mobile timeline line */}
        <div className="md:hidden absolute left-10 sm:left-11 top-0 bottom-0 w-0.5 bg-gradient-to-b from-[#935F3B]/35 via-[#85441E]/55 to-[#B26A3B]/35 pointer-events-none" />
...
function TimelineCard({ event, Icon, mobile }: { event: TimelineEvent; Icon: TimelineIcon; mobile?: boolean }) {
  return (
    <div
      className={`rounded-lg sm:rounded-xl border-2 border-[#85441E]/25 bg-[#EFBE94] backdrop-blur-sm shadow-[0_10px_24px_rgba(56,21,11,0.18)] hover:shadow-[0_14px_30px_rgba(56,21,11,0.24)] transition-all duration-300 ${
    ...
    >
      <div className={`${mobile ? "space-y-2" : "space-y-3 md:space-y-4"}`}>
        ...
        <div className="flex items-center gap-1.5">
          <Clock
            className={`${mobile ? "w-3.5 h-3.5" : "w-4 h-4 md:w-5 md:h-5"} text-[#85441E] flex-shrink-0`}
          />
          <p
            className={`${mobile ? "text-[10px]" : "text-xs sm:text-sm md:text-base"} font-bold tracking-[0.15em] text-[#85441E] uppercase`}
          >
            {event.time}
          </p>
        </div>
        ...
        {/* Title */}
        <h3
          className={`${mobile ? "text-sm sm:text-base" : "text-base sm:text-lg md:text-xl lg:text-2xl"} ${cinzel.className} font-semibold text-[#85441E] leading-tight`}
        >
          {event.title}
        </h3>
        ...
        {event.description && (
          <p
            className={`${mobile ? "text-[10px] sm:text-xs" : "text-xs sm:text-sm md:text-base"} ${cormorant.className} text-[#935F3B] leading-relaxed`}
          >
            {event.description}
          </p>
        )}
        ...
        {event.location && (
          <div
            ...
            } border-t border-[#B26A3B]/35`}
          >
            <MapPin
              className={`${mobile ? "w-3 h-3" : "w-3.5 h-3.5 md:w-4 md:h-4"} text-[#38150B] mt-0.5 flex-shrink-0`}
            />
            <p className={`${mobile ? "text-[10px]" : "text-xs md:text-sm"} ${cormorant.className} text-[#935F3B] leading-relaxed`}>
              {event.location}
            </p>
          </div>
        )}
...
function IconBadge({
  ...
  if (imageSrc) {
    return (
      <Image
        ...
        style={{
          // Tint the icon-style images to the motif color #EFBE94
          filter:
            "brightness(0) saturate(100%) invert(88%) sepia(28%) saturate(311%) hue-rotate(329deg) brightness(105%) contrast(96%)",
        }}
      />
    )
  }
  ...
  return (
    <div
      ...
      } rounded-full border-[1.5px] border-[#85441E]/80 bg-[#38150B] flex items-center justify-center shadow-md hover:shadow-lg hover:scale-105 transition-transform duration-300`}
    >
      <Icon
        className={`${
